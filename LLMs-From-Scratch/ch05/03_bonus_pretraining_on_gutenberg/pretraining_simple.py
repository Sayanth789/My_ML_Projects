"""
Script for pretrainng a small GPT-2 124M parameter model
on books from project Gutenberg.

Before running the script make sure that you downloaded and processed the dataset as described in the
README.md.
"""
...
def create_dataloaders(text_data, train_ratio, batch_size, max_length, stride, num_workers=0):
  # note that by setting num_workers=0; we disallow the parallel processing...??
...
def print_eta(start_time, book_start_time, index, total_files):
  book_end_time = time.time()   # End time for processing this book
  ...
  try:
     for epoch in range(n_epochs):
      ...
      for index, file_path in enumerate(all_files, 1):
        ...
        text_data = read_text_file(file_path) + "<|indexoftext|>"
        print(f"Tokenizing the file {index} of {total_files}: {file_path}")
...
if __name__== "__main__":  # Whatch for the intendation here it is a must 


  parser = argparse.ArgumentParser(description="GPT Model Training Configuration")
  parser.add_argument("--data_dir", type=str, default="guttenberg/data",
...
torch.save(model.state_dict(), output_dir/ "model_pg_finale.pth")
